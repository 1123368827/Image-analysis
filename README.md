...
## 项目概述
本项目包含六个图像分析实验，涵盖计算机视觉领域的多个典型应用场景。实验内容涉及传统图像处理、卷积神经网络构建、迁移学习等多个技术方向，主要技术特点包括：
...
实验数据集涵盖自然场景、生物医学、交通标识、人脸特征等多个领域，展现了深度学习在不同尺度、不同复杂度图像分析任务中的应用潜力。
...
### 1. [水质检测实验](#图像分析实验1---水质检测实验)
- ​**技术要点**：基础CNN构建、类别均衡处理、多分类评估
- ​**数据集**：5类水质图片（约250张）
- ​**核心方法**：128x128图像标准化 + 4层CNN

### 2. [德国交通标志分类](#图像分析实验2---德国交通标志分类)
- ​**技术要点**：小尺寸图像处理、高类别数分类
- ​**数据集**：43类交通标志
- ​**核心方法**：32x32图像压缩 + 浅层CNN

### 3. [人脸识别实验](#图像分析实验3---人脸识别实验)
- ​**技术要点**：传统图像处理、特征点检测
- ​**数据集**：任意人脸图像
- ​**核心方法**：Dlib预训练模型 + 68点特征定位

### 4. [番茄病害识别](#图像分析实验4---番茄数据集实验)
- ​**技术要点**：小样本优化、模型调参
- ​**数据集**：8类病叶图像
- ​**核心方法**：数据增强 + 改进CNN结构

### 5. [车辆分类实验](#图像分析实验5---车辆数据集实验)
- ​**技术要点**：数据增强策略、深度CNN设计
- ​**数据集**：10类车辆图像
- ​**核心方法**：72x72→64x64裁剪 + 双卷积层网络

### 6. [鸟类识别实验](#图像分析实验6---鸟类识别实验)
- ​**技术要点**：迁移学习、大尺度分类
- ​**数据集**：200类鸟类图像
- ​**核心方法**：VGG16微调 + 高阶特征提取
...
### 实验结果
1. **模型训练结果**
![11](https://github.com/user-attachments/assets/c2770d58-afa9-4d39-81f1-e8f13069af90)


经过10次训练，训练集准确率达到了95%，测试集准确率达到了80%。

2. **模型测试结果**
<img width="995" alt="111" src="https://github.com/user-attachments/assets/92b7b858-f3d6-42b4-a846-53a6d886749e" />
预测准确。
